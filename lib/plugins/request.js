...
function urlFilter(config) {
  const { url, params, baseURL, baseParams } = config
...
const plugin = {
  ...
  install(Vue, defaults, buildInPlugins) {
    const { interceptors: interceptorsConfig, ...config } = merge(defaultConfig, defaults)
    ...
    // 内置的请求拦截器
    interceptors.request.use(beforeRequestSend, handleRequestError)
